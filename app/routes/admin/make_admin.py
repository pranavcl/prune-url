...
from app import cur, conn, logger, INTERNAL_SERVER_ERROR_MSG
from app.helpers.check_jwt import check_jwt

make_admin_bp = Blueprint("make_admin", __name__)
GO_BACK = "<br><a href=\"/\">Return to home</a>"

@make_admin_bp.route("/make-admin", methods=["POST"])
def post_make_admin():
    ...
    try:
        ...
        cur.execute("UPDATE users SET role = 'admin' WHERE email = %s;", (email_input,))
        conn.commit()

        logger.info(f"✅ {email_input} has been made an admin")
    except Exception:
        print_exc()
        logger.error(f"⛔ Failed to make {email_input} an admin")
        return render_template("message.html", message=INTERNAL_SERVER_ERROR_MSG), 500

    return render_template("message.html", message=f"{email_input} has been made an admin.{GO_BACK}"), 200