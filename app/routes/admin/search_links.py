...
search_links_bp = Blueprint("search_links", __name__)

GO_BACK = "<br><a href=\"/\">Return to home</a>"

@search_links_bp.route("/search-links")
def get_search_links():
    ...
    sort_by = request.args.get("link_sort_by")
    sort_order = request.args.get("link_sort_order")
    page = request.args.get("page")

    # Validate input

    ALLOWED_SORT_BY = {"created", "visits"}
    ...
    if search_link:
        query += " WHERE short_url ILIKE %s OR redirect ILIKE %s"
        params.append(f"%{search_link}%")
        params.append(f"%{search_link}%")
    ...
    if search_link:
        count_query += " WHERE short_url ILIKE %s OR redirect ILIKE %s"
        count_params.append(f"%{search_link}%")
        count_params.append(f"%{search_link}%")
    ...
    return render_template("search-links.html", data_links=dumps(records), total_pages=total_pages, current_page=page)