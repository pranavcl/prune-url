...
def get_reset_password():
    ...
    if not email_input or not token:
        return f"Email or token is missing.{RETURN_HOME}", 400
    
    if len(email_input) < 6 or len(email_input) > 128:
        return f"Email must be between 6 and 128 characters in length.{RETURN_HOME}", 400
    
    if len(token) > 128:
        return f"Token is too long.{RETURN_HOME}", 400
...
def post_reset_password():
    ...
    if not email_input or not token:
        return f"Email or token is missing.{GO_BACK}", 400
    
    if len(email_input) < 6 or len(email_input) > 128:
        return f"Email must be between 6 and 128 characters in length.{GO_BACK}", 400
    
    if len(token) > 128:
        return f"Token is too long.{GO_BACK}", 400
    
    email_input = email_input.lower()
    
    if not password or not confirm_password:
        return f"Please enter a password.{GO_BACK}", 400
    
    if password != confirm_password:
        return f"Passwords must match.{GO_BACK}", 400
    
    if len(password) < 6 or len(password) > 128:
        return f"Password must be between 6 and 128 characters in length.{GO_BACK}", 400
    
    try:
        cur.execute("SELECT * FROM reset_password WHERE email = %s;", (email_input,))
        records = cur.fetchone()
        if not records:
            return f"Reset password token not found.{RETURN_HOME}", 404
        
        if datetime.datetime.now() > records[2]:
            return f"Your password reset link has expired.{RETURN_HOME}", 400
        
        token_db = records[1].encode("utf-8")
        if not checkpw(token.encode("utf-8"), token_db):
            return f"Your token is invalid.{RETURN_HOME}", 400
    except Exception:
        print_exc()
    ...
    try:
        ...
        cur.execute("UPDATE users SET password = %s", (hashedPassword.decode("utf-8"),))
        conn.commit()
    except Exception:
        print_exc()
    ...
    return f"Password reset successfully.{RETURN_HOME}"